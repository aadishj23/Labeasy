...
import { Loader2, Users, Search, Plus, UserRound, CalendarClock } from "lucide-react";
...
export default function DoctorPatients() {
  ...
  // profile dialog
  const [active, setActive] = useState<any>(null); // {patient, history}
  ...
  const openProfile = async (p: any) => {
    ...
    setBookSlotId(""); setPayStatus(""); setAmtPaid(""); setBookMsg("");
    try {
      const [profRes, slotRes] = await Promise.all([
        fetch(`/api/v1/vendor/patients/${p.id}`, { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)),
        fetch("/api/v1/doctor/slots", { cache: "no-store" }).then((r) => (r.ok ? r.json() : { slots: [] })),
      ]);
      if (profRes) setActive(profRes);
      const now = Date.now();
      setSlots((slotRes.slots || []).filter((s: any) => new Date(s.start_at).getTime() >= now && s.booked_count < s.capacity));
    } finally {
  ...
  const bookSlot = async () => {
    if (!active?.patient || !bookSlotId || !payStatus) {
      setBookMsg("Pick a slot and payment status.");
      return;
    }
    setBooking(true);
    setBookMsg("");
    try {
      const res = await fetch(`/api/v1/doctor/slots/${bookSlotId}/book`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          patient_name: active.patient.name,
          patient_phone: active.patient.phone,
      ...
      setBookMsg("Booked!");
      openProfile(active.patient); // refresh history + slots
    } finally {
      setBooking(false);
    }
  };
      ...
      <Dialog open={!!active} onOpenChange={(o) => !o && setActive(null)}>
        <DialogContent className="max-w-lg">
          ...
          <div className="rounded-xl border border-border bg-secondary/20 p-3">
            <p className="mb-2 flex items-center gap-1.5 text-sm font-medium"><CalendarClock className="h-4 w-4 text-primary" /> Book a slot</p>
            {slots.length === 0 ? (
              <p className="text-xs text-muted-foreground">No open slots. Add slots first.</p>
            ) : (
              <div className="space-y-2">
                <select className={field} value={bookSlotId} onChange={(e) => setBookSlotId(e.target.value)}>
                  <option value="">Select a slot…</option>
                  {slots.map((s) => <option key={s.id} value={s.id}>{fmtDate(s.start_at)}</option>)}
                </select>
                ...
                <Button variant="gradient" className="w-full" onClick={bookSlot} disabled={booking || !bookSlotId || !payStatus}>
